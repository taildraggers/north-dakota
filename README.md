...
Daily aggregator of North Dakota-located taildragger classified
...
repos' approach, with the state filter swapped to "ND".
...
- Each fetched page's listing table is parsed back into `(title, url,
  ...
  Location cell ends in `, ND` (case-insensitive), then de-duplicated by
...
- The result is rendered into `docs/index.html` titled **"Taildragger Ads
  in North Dakota"**, using the exact same table/card layout, dark-mode
...
4. GitHub will publish the page at `https://taildraggers.github.io/north-dakota/`
...
```html
<iframe
  src="https://taildraggers.github.io/north-dakota/"
  title="Taildragger Ads in North Dakota"
...
- This repo could be copied/adapted for other states by changing
  `STATE_ABBR` and `_NORTH_DAKOTA_LOCATION_RE` in `main.py`, and updating
  `PAGE_TITLE`.

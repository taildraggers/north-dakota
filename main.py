"""Aggregates North Dakota-located listings from every taildraggers.com
...
ones whose location ends in ", ND".
...
Jersey/New Mexico/New York/North Carolina repos' approach, with the state
filter swapped to "ND".
"""
...
PAGE_TITLE = "Taildragger Ads in North Dakota"
STATE_ABBR = "ND"
...
_NORTH_DAKOTA_LOCATION_RE = re.compile(r",\s*ND\s*$", re.IGNORECASE)


def _is_north_dakota(location: str) -> bool:
    return bool(_NORTH_DAKOTA_LOCATION_RE.search(location.strip()))
...
def collect_listings() -> list[Listing]:
    ...
    for repo in SOURCE_SITES:
        ...
        north_dakota_rows = [row for row in rows if _is_north_dakota(row.location)]
        print(f"  [{repo}] {len(rows)} listings, {len(north_dakota_rows)} in North Dakota")
        all_listings.extend(north_dakota_rows)
...
def render_html(listings: list[Listing]) -> str:
    ...
    rows_html = "".join(rows) if rows else (
        ...
        "No North Dakota listings found in the latest run.</td></tr>"
    )
...
def main() -> None:
    ...
    print(f"[main] total unique North Dakota listings: {len(listings)}")
